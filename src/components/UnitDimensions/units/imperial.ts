...
import { SquareMeter, squareMeter, subtract, Subtract } from '.';
import {
  Unit,
  unit,
  composedUnit,
...
import { Meter, meter } from './physics';

// Length
export type Twip = ConversionUnit<'Twip', Equal<0.00001763888888888889, Meter>>;
export const twip: Twip = conversionUnit(
  'Twip',
  equal(0.00001763888888888889, meter),
);
...
export type Barleycorn = ConversionUnit<
  'Barleycorn',
...
export const furlong: Furlong = conversionUnit(
  'Furlong',
  equal(201.168, meter),
);
...
export const nauticalMile: NauticalMile = conversionUnit(
  'NauticalMile',
  equal(1852, meter),
);
...
export const perch: Perch = conversionUnit(
  'Perch',
  equal(25.29285264, squareMeter),
);

export type Rood = ConversionUnit<'Rood', Equal<1011.7141056, SquareMeter>>;
export const rood: Rood = conversionUnit(
  'Rood',
  equal(1011.7141056, squareMeter),
);

export type Acre = ConversionUnit<'Acre', Equal<4046.8564224, SquareMeter>>;
export const acre: Acre = conversionUnit(
  'Acre',
  equal(4046.8564224, squareMeter),
);

export type SquareMile = ConversionUnit<
  'SquareMile',
